...
function sortMoviesByRank(numbers){
  ...
  for (let j = 0; j < numbers.length - 1; j++) {
      ...
      for (let i = j; i < numbers.length; i++) {
          if (numbers[i].rank > max_num.rank) {
 ...
 */
function sortMoviesByAttr(numbers, sortAttr){
  // CODE GOES HERE

  for (let j = 0; j < numbers.length - 1; j++) {

    maxmov = getMaxMovieObject(numbers,j,sortAttr)
    max_num = maxmov.max_number
    max_ind = maxmov.max_index

    // let max_num = numbers[j];
    // let max_location = j;

    // for (let i = j; i < numbers.length; i++) {
    //     if (numbers[i][sortAttr] > max_num[sortAttr]) {
    //         // Know max AND it's index (location)
    //         max_num = numbers[i]
    //         max_location = i
    //     }
